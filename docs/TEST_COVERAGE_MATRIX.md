...
| `src/current_time.rs` | Superseded duplicate of the `current_time` tool (not declared in `lib.rs`; the shipped implementation is `CurrentTimeTool` in `src/tools.rs`, briefly swapped for this module by 416cabe9 and restored by the revert 74490cb8 on 2026-09-02); file kept only until its deletion is approved | Not compiled; the live tool is covered by `src/tools.rs` unit tests, the CLI default-list goldens, and `tests/readme_tool_inventory.rs`. |
