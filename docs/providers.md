...
| `copilot` | `github-copilot`, `copilot` | GitHub Copilot Chat Completions, Responses, or Anthropic Messages path selected by model metadata. |
...
| `github-copilot` | `copilot` | text + tool-calls | Copilot Chat Completions / Responses / Anthropic Messages | Token-exchange metadata (`https://api.githubcopilot.com` fallback) | `Authorization: Bearer` (`GITHUB_COPILOT_API_KEY`, `GITHUB_TOKEN`) | `native-adapter-required` | VCR-verified chat scenarios + transport-routing tests | [metadata](../tests/provider_metadata_comprehensive.rs), [native-verify](../tests/provider_native_verify.rs), [cassette](../tests/fixtures/vcr/verify_copilot_simple_text.json), [parity-report](provider-native-parity-report.json) |
...
| `copilot` | `github-copilot` | Copilot multi-transport | `GITHUB_COPILOT_API_KEY`, `GITHUB_TOKEN` | Short form; `github-copilot` is canonical. |
