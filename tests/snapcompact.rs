...
use pi::compaction_snap::{
    COMPACTION_SUMMARY_PREFIX, SnapFrame, SnapPayload, attach_frames, frames_from_details,
    png_encode, render_frames, strip_snapcompact_images,
};
use pi::model::{
...
fn attach_frames_places_images_after_text_block() {
    ...
    assert_eq!(attach_frames(base, None), base_with_text());
}
    ...
    let summary = Message::User(UserMessage {
        content: UserContent::Blocks(vec![
            ContentBlock::Text(TextContent::new(format!(
                "{COMPACTION_SUMMARY_PREFIX}s{COMPACTION_SUMMARY_SUFFIX}"
            ))),
