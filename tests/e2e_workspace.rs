...
fn multi_root_registry_read_spans_roots_and_denies_outside() {
    let harness = TestHarness::new("multi_root_read_span_and_deny");
    ...
    let mut handle = WorkspaceHandle::single(primary.path());
    ...
    asupersync::test_utils::run_test(|| {
        ...
        async move {
            ...
            assert!(
                rendered.contains("extra-content"),
                "extra-root read must return file content, got: {rendered}"
            );
        }
    });
    harness
        .log()
        .info("multi-root", "extra-root read verified");
...
fn shared_handle_removal_revokes_across_clones() {
    let harness = TestHarness::new("shared_handle_revocation");
    let primary = tempfile::tempdir().expect("primary tempdir");
    let extra = tempfile::tempdir().expect("extra tempdir");
    ...
    let snapshot = cloned.snapshot_or(primary.path());
    assert!(
