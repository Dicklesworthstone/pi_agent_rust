...
fn ensure_committed_paths_match_worktree(
    ...
    if !differing.is_empty() {
        ...
        return Err(format!(
            "must-pass source inputs differ in the worktree from the commit they are being bound to; commit them before collecting release evidence: {}",
            differing.join(", ")
        ));
    }
...
fn ensure_index_matches_commit_when_readable(
    ...
    match status.code() {
        Some(0) => Ok(()),
        Some(1) => Err(
            "must-pass source inputs differ in the index; commit them before collecting release evidence"
                .to_string(),
        ),
        code => Err(format!(
