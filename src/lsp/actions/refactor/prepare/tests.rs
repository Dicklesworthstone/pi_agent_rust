...
fn concrete_ranges_preserve_unicode_text_and_cursor_at_token_end() {
    for cursor in [2, 4, 7] {
        let target = parse(&range(2, 7), "😀alpha\r\nbeta", position(0, cursor)).unwrap().unwrap();
...
fn multiline_ranges_keep_original_line_terminators() {
    ...
    let target = parse(&raw, "😀alpha\r\nbeta\n", position(1, 2)).unwrap().unwrap();
    assert_eq!(target.source_text, "alpha\r\nbeta");
}

#[test]
fn null_preparation_is_an_explicit_refusal() {
    assert!(parse(&Value::Null, "alpha", position(0, 2)).unwrap().is_none());
}

#[test]
fn malformed_or_unnegotiated_preparation_never_becomes_a_range() {
    for raw in [
        json!(false), json!([]), json!({}), json!({"defaultBehavior":true}),
        json!({"defaultBehavior":false}), json!({"range":range(0, 5)}),
...
fn nonexact_empty_reversed_and_unrelated_ranges_are_rejected() {
    for raw in [range(1, 7), range(2, 99), range(4, 4), range(7, 2), range(5, 7),
...
fn preparation_bounds_both_placeholder_and_original_text() {
    ...
    assert!(parse(&raw, "alpha", position(0, 3)).unwrap_err().to_string().contains("LSP_RENAME_LIMIT"));
    let text = "x".repeat(MAX_TARGET_BYTES + 1);
    assert!(parse(&range(0, u32::try_from(text.len()).unwrap()), &text, position(0, 0)).is_err());
    let raw = json!({"extra":"x".repeat(super::super::MAX_ACTION_BYTES + 1)});
    assert!(parse(&raw, "alpha", position(0, 1)).unwrap_err().to_string().contains("LSP_EDIT_LIMIT"));
}
...
fn preparation_requires_an_explicit_valid_capability() {
    ...
    for caps in [json!({}), json!({"renameProvider":true}), json!({"renameProvider":{}}),
        json!({"renameProvider":{"prepareProvider":false}})]
    {
        assert!(!supports_prepare(&caps).unwrap());
    }
    for caps in [json!({"renameProvider":false}), json!({"renameProvider":1}),
...
fn targeting_accepts_one_position_selector_without_guessing() {
    for action in ["rename", "prepare_rename"] {
        for selector in [json!({"position":{"line":0,"character":5}}),
            json!({"symbol":"alpha#2"}), json!({"symbol":"alpha","line":1})]
        {
            let mut raw = json!({"action":action,"file":"a.rs"});
            raw.as_object_mut().unwrap().extend(selector.as_object().unwrap().clone());
            if action == "rename" { raw["newName"] = json!("renamed"); }
            validate_input(&serde_json::from_value(raw).unwrap()).unwrap();
        }
    }
    for extra in [json!({}), json!({"line":1}), json!({"symbol":""}),
...
fn inspection_cannot_accept_mutation_or_unrelated_selectors() {
    for (key, value) in [
        ("apply",json!(false)), ("apply",json!(true)), ("newName",json!("renamed")),
        ("query",json!("1")), ("actionId",json!("a")), ("refactorId",json!("r")),
        ("newFile",json!("b.rs")), ("method",json!("workspace/executeCommand")),
        ("payload",json!({})), ("range",range(0,5)), ("only",json!(["refactor"])),
        ("completionId",json!("c")), ("hierarchyId",json!("h")),
        ("snippetValues",json!({})), ("resolve",json!(false)), ("limit",json!(1)),
        ("after",json!("x")), ("formatOptions",json!({})),
    ] {
        let mut raw = json!({"action":"prepare_rename","file":"a.rs","symbol":"alpha"});
        raw[key] = value;
        assert!(validate_input(&serde_json::from_value(raw).unwrap()).is_err(), "{key}");
    }
}
...
fn sources_are_bounded_before_reading_and_special_files_are_rejected() {
    ...
    std::fs::File::create(&large).unwrap().set_len(MAX_SOURCE_BYTES as u64 + 1).unwrap();
    assert!(read_source(&large).unwrap_err().to_string().contains("LSP_RENAME_LIMIT"));
