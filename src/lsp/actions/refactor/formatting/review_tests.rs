...
fn fixture(root: &Path, startup: &str) -> Option<LspTool> {
    ...
    let Some(python) = python else {
        assert!(std::env::var_os("PI_LSP_REQUIRE_PROTOCOL").is_none(), "Python required for reviewed formatting tests");
    ...
    let config = Config {
        lsp: Some(LspSettings {
            servers: Some(HashMap::from([("review-format-peer".to_string(), LspServerSettings {
                command: Some(python.display().to_string()),
                args: Some(vec!["-I".into(), "-u".into(), server.display().to_string(), startup.into()]),
...
fn mode(root: &Path, value: &str) {
    std::fs::write(root.join("review-mode.json"), serde_json::to_string(value).unwrap()).unwrap();
}
...
fn calls(root: &Path) -> Vec<Value> {
    std::fs::read_to_string(root.join("review-requests.jsonl")).unwrap().lines()
...
fn count(root: &Path, method: &str) -> usize {
    ...
    calls(root).iter().filter(|frame| frame["method"] == method).count()
}
...
fn assert_original(root: &Path) {
    ...
    assert_eq!(std::fs::read_to_string(root.join("other.reviewfmt")).unwrap(), "untouched\n");
}
...
fn review_inspect_approve_uses_one_formatter_response_and_consumes_the_plan() {
    ...
    assert_eq!(reviewed["workspaceEdit"]["documentChanges"][0]["edits"][0]["newText"], "let x = 1;");
    ...
    assert!(run(&tool, &rt, approve(id)).unwrap_err().to_string().contains("LSP_REFACTOR_STALE"));
...
fn range_approval_retains_unicode_selection_and_format_options() {
    ...
    let request = requests.iter().find(|frame| frame["method"] == "textDocument/rangeFormatting").unwrap();
...
fn approved_noops_are_consumed_without_rewriting_original_files() {
    for setting in ["null", "empty", "same"] {
        ...
        let before = std::fs::metadata(temp.path().join("source.reviewfmt")).unwrap().modified().unwrap();
        ...
        assert_eq!(before, std::fs::metadata(temp.path().join("source.reviewfmt")).unwrap().modified().unwrap());
        assert_original(temp.path());
        assert!(run(&tool, &rt, approve(&reviewed["refactorId"])).is_err());
    }
}
...
fn same_text_close_reopen_invalidates_versioned_and_unversioned_previews() {
    for startup in ["normal", "nosync"] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(), startup) else { return };
        let rt = runtime();
        let reviewed = run(&tool, &rt, input()).unwrap();
...
fn selectors_and_other_actions_cannot_override_or_consume_a_valid_format_plan() {
    ...
    let reviewed = run(&tool, &rt, input()).unwrap();
    for extra in [
        json!({"file":"other.reviewfmt"}), json!({"formatOptions":{"tabSize":2}}),
        json!({"range":{"start":{"line":0,"character":0},"end":{"line":0,"character":1}}}),
        json!({"action":"rename"}), json!({"query":"new action"}),
    ] {
...
fn invalid_fresh_requests_leave_the_existing_preview_usable() {
    ...
    let rt = runtime();
    let reviewed = run(&tool, &rt, input()).unwrap();
    for extra in [json!({"formatOptions":{"tabSize":0}}), json!({"query":"other"}),
...
fn full_edit_remains_reviewable_when_the_overview_is_shortened() {
    ...
    assert_eq!(reviewed["workspaceEditComplete"], true);
    assert_eq!(reviewed["workspaceEdit"]["documentChanges"][0]["edits"][0]["newText"].as_str().unwrap().len(), 70000);
    run(&tool, &rt, approve(&reviewed["refactorId"])).unwrap();
...
fn failed_or_oversized_fresh_computation_retires_previous_approval() {
    for setting in ["error", "invalid", "overlap", "preview-limit", "response-limit"] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(), "normal") else { return };
        let rt = runtime();
        let reviewed = run(&tool, &rt, input()).unwrap();
        ...
        if setting == "preview-limit" {
            assert!(error.to_string().contains("LSP_EDIT_LIMIT"), "{setting}: {error}");
        }
        assert!(run(&tool, &rt, approve(&reviewed["refactorId"])).unwrap_err().to_string().contains("LSP_REFACTOR_STALE"));
        assert_original(temp.path());
        assert!(!std::fs::read_dir(temp.path()).unwrap().any(|entry| entry.unwrap().file_name().to_string_lossy().starts_with(".pi-lsp-")));
    }
}
...
fn preview_never_opens_command_or_unsolicited_edit_permission() {
    ...
    let reviewed = run(&tool, &rt, input()).unwrap();
    let reply: Value = serde_json::from_str(&std::fs::read_to_string(temp.path().join("format-probe.json")).unwrap()).unwrap();
    assert_eq!(reply["result"]["applied"], false);
    assert_original(temp.path());
    run(&tool, &rt, approve(&reviewed["refactorId"])).unwrap();
    assert_eq!(std::fs::read_to_string(temp.path().join("other.reviewfmt")).unwrap(), "untouched\n");
    assert_eq!(count(temp.path(), "workspace/executeCommand"), 0);
}
...
fn restricted_formatting_is_rejected_before_startup_and_cannot_consume_approval() {
    ...
    rt.block_on(async {
        let restricted = asupersync::Cx::for_request().restrict::<asupersync::cx::cap::None>();
        let _guard = restricted.set_current_restricted();
        let error = tool.execute("restricted-approval", approve(&reviewed["refactorId"]), None).await.unwrap_err();
        assert!(error.to_string().contains("LSP_EDIT_PERMISSION"), "{error}");
    });
    run(&tool, &rt, approve(&reviewed["refactorId"])).unwrap();
}
    ...
    rt.block_on(async {
        ...
        for _ in 0..1000 {
            ...
            if let Ok(log) = std::fs::read_to_string(temp.path().join("review-requests.jsonl"))
                && log.lines().any(|line| serde_json::from_str::<Value>(line).is_ok_and(|frame| frame["method"] == "textDocument/formatting"))
            {
                posted = true;
                break;
            }
        ...
        assert!(posted);
        drop(pending);
        tool.execute("barrier", json!({"action":"request","file":"source.reviewfmt","method":"test/count","timeout":5}), None).await.unwrap();
    });
    let frames = calls(temp.path());
    let request = frames.iter().find(|frame| frame["method"] == "textDocument/formatting").unwrap();
    let cancel = frames.iter().find(|frame| frame["method"] == "$/cancelRequest").unwrap();
...
fn startup_is_inside_the_format_request_budget() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(), "startup-hang") else { return };
    let mut args = input();
    args["timeout"] = json!(1);
...
fn format_budget_does_not_restart_and_checks_cancellation() {
    ...
    budget.started = Instant::now().checked_sub(Duration::from_secs(7200)).unwrap();
    assert!(budget.remaining().unwrap_err().to_string().contains("LSP_TIMEOUT"));
    let budget = FormatBudget::new(Duration::from_secs(10));
    budget.owner.cancel_with(asupersync::types::CancelKind::User, Some("cancel formatting"));
...
fn fresh_format_rejects_unrelated_selectors_before_reading_any_source() {
    ...
    let rt = runtime();
    for extra in [json!({"query":"1"}), json!({"limit":1}), json!({"newName":"x"}),
        json!({"newFile":"x"}), json!({"actionId":"x"}), json!({"hierarchyId":"x"}),
        json!({"method":"workspace/executeCommand"}), json!({"payload":{}})] {
        let mut args = input();
        args.as_object_mut().unwrap().extend(extra.as_object().unwrap().clone());
        assert!(run(&tool, &rt, args).unwrap_err().to_string().contains("LSP_USAGE"));
    }
