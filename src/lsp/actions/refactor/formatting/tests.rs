...
fn fixture(root: &Path, mode: &str) -> Option<(LspTool, asupersync::runtime::Runtime)> {
    ...
    let Some(python) = python else {
        assert!(std::env::var_os("PI_LSP_REQUIRE_PROTOCOL").is_none(), "Python required for formatting protocol tests");
    ...
    let config = Config {
        lsp: Some(LspSettings {
            servers: Some(HashMap::from([("format-fixture".to_string(), LspServerSettings {
...
fn document_format_previews_without_writing_and_applies_only_when_requested() {
    ...
    let preview = run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture"})).unwrap();
    assert_eq!(preview["previewOnly"], true);
    assert_eq!(preview["applied"], false);
    assert_eq!(preview["changed"], true);
    assert_eq!(preview["edits"][0]["newText"], "let x = 1;");
    assert_eq!(std::fs::read(temp.path().join("source.fmtfixture")).unwrap(), b"let x=1;\n");
    let applied = run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":true})).unwrap();
    assert_eq!(applied["applied"], true);
    assert_eq!(std::fs::read(temp.path().join("source.fmtfixture")).unwrap(), b"let x = 1;\n");
    let calls = requests(temp.path());
    let formats: Vec<_> = calls.iter().filter(|call| call["method"] == "textDocument/formatting").collect();
    assert_eq!(formats.len(), 2, "apply makes a new request, not a stale preview replay");
    assert_eq!(formats[0]["params"]["options"], json!({"tabSize":4,"insertSpaces":true}));
}

#[test]
fn range_format_preserves_utf16_selection_and_caller_options() {
    let temp = tempfile::tempdir().unwrap();
    let Some((tool, runtime)) = fixture(temp.path(), "range") else { return };
    ...
    assert_eq!(std::fs::read_to_string(temp.path().join("source.fmtfixture")).unwrap(), "😀y\rnext\r\n");
    let calls = requests(temp.path());
    let call = calls.iter().find(|call| call["method"] == "textDocument/rangeFormatting").unwrap();
...
fn invalid_selection_is_rejected_before_server_startup() {
    ...
    for range in [
        ...
        assert!(run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","range":range})).is_err());
    }
...
fn malformed_options_are_not_sent_or_silently_ignored() {
    for raw in [json!(true), json!([]), json!({"tabSize":0}), json!({"tabSize":33}),
        json!({"insertSpaces":"yes"}), json!({"insertFinalNewline":1}),
        json!({"custom":null}), json!({"custom":1.5}), json!({"custom":{}})]
    {
        assert!(options(Some(&raw)).is_err(), "{raw}");
    }
    assert_eq!(options(Some(&json!({"tabSize":8}))).unwrap(), json!({"tabSize":8,"insertSpaces":true}));
}

#[test]
fn unadvertised_formatting_is_an_explicit_error_without_dispatch() {
    let temp = tempfile::tempdir().unwrap();
    let Some((tool, runtime)) = fixture(temp.path(), "unsupported") else { return };
    let error = run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":true})).unwrap_err();
    assert!(error.to_string().contains("LSP_FORMAT_UNSUPPORTED"));
    assert!(!requests(temp.path()).iter().any(|call| call["method"] == "textDocument/formatting"));
}
...
fn invalid_or_failed_formatter_responses_never_modify_the_source() {
    for mode in ["invalid", "overlap", "bad-line", "error", "oversized"] {
        ...
        assert!(run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":true})).is_err(), "{mode}");
        assert_eq!(std::fs::read(temp.path().join("source.fmtfixture")).unwrap(), b"let x=1;\n");
    }
}
...
fn preview_and_apply_reject_source_drift_while_the_server_works() {
    for apply in [false, true] {
        let temp = tempfile::tempdir().unwrap();
        let Some((tool, runtime)) = fixture(temp.path(), "drift") else { return };
        let error = run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":apply})).unwrap_err();
        assert!(error.to_string().contains("LSP_EDIT_CONFLICT"));
        assert_eq!(std::fs::read(temp.path().join("source.fmtfixture")).unwrap(), b"external edit\n");
    }
}
...
fn empty_and_identical_results_do_not_rewrite_the_file() {
    for mode in ["null", "empty", "same"] {
        let temp = tempfile::tempdir().unwrap();
        let Some((tool, runtime)) = fixture(temp.path(), mode) else { return };
        let result = run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":true})).unwrap();
        assert_eq!(result["changed"], false);
        assert_eq!(result["applied"], false);
        assert_eq!(result["returnedNull"], mode == "null");
        assert!(!requests(temp.path()).iter().any(|call| call["method"] == "textDocument/didClose"));
        assert_eq!(std::fs::read(temp.path().join("source.fmtfixture")).unwrap(), b"let x=1;\n");
    }
}
...
fn actual_formatting_keeps_same_position_insert_order() {
    let temp = tempfile::tempdir().unwrap();
    let Some((tool, runtime)) = fixture(temp.path(), "inserts") else { return };
    run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":true})).unwrap();
    assert_eq!(std::fs::read(temp.path().join("source.fmtfixture")).unwrap(), b"ABClet x=1;\n");
}

#[test]
fn preview_limits_never_return_a_partial_replacement_as_an_edit() {
    let temp = tempfile::tempdir().unwrap();
    let Some((tool, runtime)) = fixture(temp.path(), "large-preview") else { return };
    let result = run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture"})).unwrap();
...
fn formatting_does_not_grant_workspace_apply_edit_permission() {
    let temp = tempfile::tempdir().unwrap();
    let Some((tool, runtime)) = fixture(temp.path(), "callback") else { return };
    run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":true})).unwrap();
    ...
    assert_eq!(std::fs::read(temp.path().join("other.fmtfixture")).unwrap(), b"keep\n");
}

#[test]
fn a_formatter_timeout_cannot_apply_or_report_a_successful_format() {
    let temp = tempfile::tempdir().unwrap();
    let Some((tool, runtime)) = fixture(temp.path(), "hang") else { return };
    let error = run(&tool, &runtime, json!({"action":"format","file":"source.fmtfixture","apply":true,"timeout":1})).unwrap_err();
...
fn formatter_edits_cannot_round_a_surrogate_interior_into_a_valid_edit() {
    let temp = tempfile::tempdir().unwrap();
    let Some((tool, runtime)) = fixture(temp.path(), "surrogate") else { return };
...
fn dropped_formatting_request_cancels_before_a_successor_can_apply() {
    ...
    runtime.block_on(async {
        let mut pending = Box::pin(tool.execute("cancel-format", json!({
        ...
        for _ in 0..500 {
            ...
            if let Ok(log) = std::fs::read_to_string(temp.path().join("requests.jsonl"))
                && log.lines().any(|line| {
                    serde_json::from_str::<Value>(line).is_ok_and(|frame| frame["method"] == "textDocument/formatting")
                })
            {
                posted = true;
                break;
        ...
        tool.execute("after-format", json!({
            "action":"request","file":"source.fmtfixture","method":"test/barrier"
        }), None).await.unwrap();
    });
    let calls = requests(temp.path());
    let posted = calls.iter().position(|frame| frame["method"] == "textDocument/formatting").unwrap();
    let cancelled = calls.iter().position(|frame| frame["method"] == "$/cancelRequest").unwrap();
    let successor = calls.iter().position(|frame| frame["method"] == "test/barrier").unwrap();
