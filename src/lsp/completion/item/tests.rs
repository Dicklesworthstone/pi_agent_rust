...
fn position(character: u32) -> Position { Position { line: 0, character } }
...
fn completion() -> Value { json!({"label":"alpha","textEdit":edit(0, 2, "alpha")}) }

#[test]
fn plain_primary_edit_uses_server_range_not_caller_fallback() {
    let edits = edits(&completion(), "al rest", position(2), Some(Range { start:position(2), end:position(2) })).unwrap();
...
fn omitted_text_edit_requires_explicit_language_independent_replacement() {
    ...
    assert!(edits(&item, "al", position(2), None).unwrap_err().to_string().contains("RANGE_REQUIRED"));
    let fallback = Range { start:position(0), end:position(2) };
    assert_eq!(edits(&item, "al", position(2), Some(fallback)).unwrap(), vec![edit(0, 2, "alpha")]);
}
...
fn default_edit_range_uses_text_edit_text_or_label_not_insert_text() {
    ...
    let item = materialize(&json!({"label":"alpha","insertText":"wrong","textEditText":"right"}), Some(&defaults)).unwrap();
    assert_eq!(item["textEdit"], edit(0,2,"right"));
    assert_eq!(item["data"], defaults["data"]);
    let item = materialize(&json!({"label":"alpha","insertText":"wrong"}), Some(&defaults)).unwrap();
    assert_eq!(item["textEdit"], edit(0,2,"alpha"));
}

#[test]
fn explicit_item_properties_override_defaults_and_preserve_opaque_data() {
    let item = materialize(&json!({"label":"x","data":{"id":"item"},"insertTextFormat":1,"textEdit":edit(0,1,"x")}),
        Some(&json!({"editRange":range(0,2),"insertTextFormat":2,"data":{"id":"default"}}))).unwrap();
...
fn insert_replace_edit_defaults_select_the_complete_replace_span() {
    let item = materialize(&json!({"label":"alpha"}), Some(&json!({"editRange":{"insert":range(0,2),"replace":range(0,4)}}))).unwrap();
    assert_eq!(edits(&item,"alxx",position(2),None).unwrap(), vec![edit(0,4,"alpha")]);
}

#[test]
fn invalid_insert_replace_relations_cannot_choose_an_unrelated_span() {
    for (insert, replace) in [(range(1,2),range(0,4)),(range(0,4),range(0,2)),(range(0,1),range(0,4))] {
...
fn auto_import_edits_are_preserved_in_original_document_coordinates() {
    ...
    let got = edits(&item,"// comment\nTy",Position { line:1,character:2 },None).unwrap();
    assert_eq!(got.len(),2);
...
fn overlaps_including_duplicate_insert_points_are_rejected() {
    for extra in [vec![edit(1,3,"bad")],vec![edit(0,0,"bad")],vec![edit(4,4,"one"),edit(4,4,"two")]] {
...
fn annotated_or_resource_shaped_additional_edits_are_not_accepted() {
    for extra in [json!({"range":range(4,4),"newText":"x","uri":"file:///outside"}),
        json!({"kind":"delete","uri":"file:///outside"}),
        json!({"range":range(4,4),"newText":"x","annotationId":"confirm"})] {
        let mut item=completion(); item["additionalTextEdits"]=json!([extra]);
        assert!(edits(&item,"al rest",position(2),None).is_err());
    }
}
...
fn snippet_command_and_indentation_modes_fail_before_application() {
    for extra in [json!({"insertTextFormat":2}),json!({"insertTextMode":2}),json!({"command":{"command":"server.exec"}})] {
        let mut item=completion();
        item.as_object_mut().unwrap().extend(extra.as_object().unwrap().clone());
        assert!(edits(&item,"al",position(2),None).unwrap_err().to_string().contains("UNSUPPORTED"));
    }
}
...
fn lazy_resolve_cannot_substitute_insertion_identity_or_commands() {
    let original=completion();
    for (property,value) in [("label",json!("other")),("textEdit",edit(0,2,"other")),
        ("filterText",json!("other")),("data",json!({"id":"other"})),
        ("command",json!({"command":"server.exec"}))] {
        let mut after=original.clone(); after[property]=value;
        assert!(resolved(&original,&after).is_err(),"{property}");
    }
    let mut after=original.clone(); after.as_object_mut().unwrap().remove("textEdit");
...
fn additional_edits_and_wire_items_have_admission_bounds() {
    ...
    assert!(edits(&item,"al rest",position(2),None).unwrap_err().to_string().contains("LIMIT"));
    assert!(materialize(&json!({"label":"x","data":"x".repeat(MAX_ITEM_BYTES)}),None).is_err());
    assert!(materialize(&json!({"label":"x"}),Some(&json!({"commitCharacters":["."]}))).is_err());
}
