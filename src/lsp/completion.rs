...
use super::{LspInput, LspTool, MAX_PAYLOAD_BYTES, display_path, resolve_tool_path, text_output, tool_err};
...
fn stale() -> Error {
    tool_err("LSP_COMPLETION_STALE", "completion source, listing or server changed; request a new listing")
}
...
fn bounded_size(value: &impl serde::Serialize, limit: usize) -> Result<usize> {
    ...
    serde_json::to_writer(&mut budget, value)
        .map_err(|_| tool_err("LSP_COMPLETION_LIMIT", "completion exceeds its serialized byte limit"))?;
    Ok(limit - budget.0)
}
...
fn read_source(path: &Path) -> Result<String> {
    ...
    if !meta.is_file() || meta.file_type().is_symlink() {
        return Err(tool_err("LSP_FILE_UNREADABLE", "completion requires a regular, nonsymlink source file"));
    }
    if meta.len() > MAX_SOURCE_BYTES as u64 {
        return Err(tool_err("LSP_COMPLETION_LIMIT", "completion source exceeds 2 MiB"));
    }
    let mut text = String::new();
    std::fs::File::open(path)?.take((MAX_SOURCE_BYTES + 1) as u64).read_to_string(&mut text)
        .map_err(|error| tool_err("LSP_FILE_UNREADABLE", format!("cannot read completion source: {error}")))?;
...
impl Budget {
    ...
    fn remaining(&self) -> Result<Duration> {
        self.owner.checkpoint().map_err(|_| tool_err("LSP_CANCELLED", "completion cancelled"))?;
        if !self.owner.capabilities().io {
            return Err(tool_err("LSP_EDIT_PERMISSION", "completion owner does not permit filesystem I/O"));
        }
        self.timeout.checked_sub(self.started.elapsed()).filter(|remaining| !remaining.is_zero())
...
impl Source {
    fn verify(&self) -> Result<Arc<ServerEntry>> {
        let entry = self.entry.upgrade().filter(|entry| entry.client.is_alive()).ok_or_else(stale)?;
        if self.created.elapsed() >= HANDLE_AGE || !self.path.starts_with(entry.client.root()) {
            return Err(stale());
        }
        let text = entry.client.synchronized_text(&self.uri).ok_or_else(stale)?;
        if !Arc::ptr_eq(&self.text, &text) {
            return Err(stale());
        }
        if let Some(before) = self.document {
            let now = entry.client.document_snapshots();
            if !now.get(&self.path).is_some_and(|now| now.version == before.version && now.hash == before.hash) {
                return Err(stale());
            }
        }
...
impl CompletionCache {
    ...
    fn start(&self) -> Result<u64> {
        ...
        state.items.clear();
        state.serial = state.serial.checked_add(1)
            .ok_or_else(|| tool_err("LSP_COMPLETION_LIMIT", "completion identity space exhausted"))?;
        Ok(state.serial)
    }
}
...
fn list_parts(raw: &Value) -> Result<(&[Value], Option<&Value>, bool)> {
    ...
    let (items, defaults, incomplete) = match raw {
        ...
        Value::Object(object) => {
            ...
            let incomplete = object.get("isIncomplete").and_then(Value::as_bool)
                .ok_or_else(|| malformed("completion list needs boolean isIncomplete"))?;
            (items.as_slice(), object.get("itemDefaults"), incomplete)
        }
        _ => return Err(malformed("completion result must be null, an array or a CompletionList")),
    };
    if items.len() > MAX_SERVER_ITEMS {
        return Err(tool_err("LSP_COMPLETION_LIMIT", "server returned more than 4096 completions"));
    }
    for item in items {
        item::label(item)?;
        for key in ["sortText", "filterText"] {
            if item.get(key).is_some_and(|value| !value.is_null() && !value.is_string()) {
                return Err(malformed("completion sortText and filterText must be strings"));
            }
        }
    }
    if let Some(defaults) = defaults {
        let defaults = defaults.as_object().ok_or_else(|| malformed("itemDefaults must be an object"))?;
        if defaults.keys().any(|key| !matches!(key.as_str(), "editRange" | "insertTextFormat" | "insertTextMode" | "data")) {
...
fn clipped(value: Option<&Value>, limit: usize) -> Value {
    let Some(text) = value.and_then(Value::as_str) else { return Value::Null };
    if text.len() <= limit { return json!(text) }
    let mut end = limit;
...
fn validate_input(input: &LspInput) -> Result<()> {
    ...
    {
        return Err(tool_err("LSP_USAGE", "completion uses file + position, or completionId; unrelated selectors are not accepted"));
    }
    if let Some(id) = &input.completion_id {
        if id.is_empty() || id.len() > 128 || input.file.is_some() || input.position.is_some()
            || input.range.is_some() || input.query.is_some() || input.limit.is_some()
        {
            return Err(tool_err("LSP_USAGE", "completionId already identifies its source and selection; do not combine it with listing selectors"));
        }
    } else if input.file.is_none() || input.position.is_none() || input.apply == Some(true)
        || input.limit == Some(0) || input.query.as_ref().is_some_and(|query| query.len() > 128)
    {
        return Err(tool_err("LSP_USAGE", "list completions with file and exact position; optional query is a case-sensitive prefix up to 128 bytes; apply requires completionId"));
    }
    Ok(())
}
...
impl LspTool {
    ...
    async fn list_completions(&self, input: &LspInput, budget: &Budget) -> Result<ToolOutput> {
        ...
        let requested = resolve_tool_path(input.file.as_deref().ok_or_else(|| malformed("missing file"))?, &self.cwd);
        ...
        let position = input.position.ok_or_else(|| malformed("missing position"))?;
        if position_to_offset_exact(&text, position).is_none() {
            return Err(tool_err("LSP_USAGE", "completion position must be an exact zero-based UTF-16 boundary"));
        }
        ...
        let (uri, entry) = asupersync::time::timeout(now, budget.remaining()?, self.synced(&path))
            .await.map_err(|_| tool_err("LSP_TIMEOUT", "completion server startup exceeded the request budget"))??;
        let caps = entry.client.capabilities().raw;
        if caps.get("positionEncoding").is_some_and(|encoding| encoding.as_str() != Some("utf-16")) {
            return Err(tool_err("LSP_UNSUPPORTED", "completion server selected an unadvertised position encoding"));
        }
        let provider = caps.get("completionProvider").and_then(Value::as_object)
            .ok_or_else(|| tool_err("LSP_UNSUPPORTED", "server did not advertise completionProvider"))?;
        ...
        let synced = entry.client.synchronized_text(&uri).filter(|synced| synced.as_ref() == text.as_str())
            .ok_or_else(stale)?;
        let document = entry.client.document_snapshots().get(&path).copied();
        let source = Arc::new(Source {
            entry: Arc::downgrade(&entry), path, uri, hash: content_hash_for_drift(&text),
            text: synced, document, position, fallback: input.range, created: Instant::now(),
        });
        source.verify()?;
        ...
        let mut matching: Vec<_> = items.iter().filter(|item| {
            item.get("filterText").and_then(Value::as_str).unwrap_or_else(|| item["label"].as_str().unwrap_or(""))
                .starts_with(prefix)
        }).collect();
        ...
        for (index, raw) in matching.into_iter().take(limit).enumerate() {
            ...
            retained.insert(id, Candidate { source: Arc::clone(&source), item, resolve });
        }
        budget.remaining()?;
        source.verify()?;
    ...
    async fn select_completion(&self, id: &str, apply: bool, budget: &Budget) -> Result<ToolOutput> {
        ...
        if selected.resolve {
            let raw = entry.client.call("completionItem/resolve", selected.item.clone(), budget.remaining()?).await?;
            budget.remaining()?;
        ...
        let documentation = selected.item.get("documentation").and_then(|value| {
            if value.is_string() { Some(value) } else { value.get("value") }
        });
        if !apply {
            let payload = json!({"action":"completion","completionId":id,"applied":false,
