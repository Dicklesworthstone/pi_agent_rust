...
mod tests {
    ...
    fn position_offset_roundtrip() {
        ...
        // Out of range line.
        assert_eq!(position_to_offset(content, Position { line: 99, character: 0 }), None);
    }
    ...
    fn position_maps_crlf() {
        let content = "ab\r\ncd\r\n";
        assert_eq!(
            position_to_offset(content, Position { line: 1, character: 2 }),
            Some(7)
        );
        assert_eq!(
    ...
    fn apply_edits_insert_at_same_point_is_deterministic() {
        ...
        let edits = vec![
            TextEdit { range, new_text: "X".into() },
            TextEdit { range, new_text: "Y".into() },
        ];
        let out = apply_text_edits(content, &edits).expect("apply");
