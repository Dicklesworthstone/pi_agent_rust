...
fn revision() -> Revision { Revision { version: 1, hash: 7 } }

fn peer(root: &Path) -> Option<Fixture> {
    Fixture::connect(root, json!({
        "textDocumentSync":1,
...
fn refresh(peer: &Fixture, uri: &str) -> Result<()> {
    peer.runtime.block_on(peer.client.refresh_document_diagnostics(uri, Duration::from_secs(5)))
}

#[test]
fn full_and_unchanged_reports_preserve_opaque_ids_and_require_a_baseline() {
    let full = parse_report(&json!({"kind":"full","resultId":"","items":[diagnostic("broken")]}), revision(), None).unwrap();
    assert_eq!(full.result_id.as_deref(), Some(""));
    let unchanged = parse_report(&json!({"kind":"unchanged","resultId":"next +/="}), revision(), Some(&full)).unwrap();
    assert!(Arc::ptr_eq(&full.items, &unchanged.items));
    assert_eq!(unchanged.result_id.as_deref(), Some("next +/="));
    assert!(parse_report(&json!({"kind":"unchanged","resultId":"id"}), revision(), None).is_err());
    let other = Revision { version: 2, ..revision() };
    assert!(parse_report(&json!({"kind":"unchanged","resultId":"id"}), other, Some(&full)).is_err());
    let no_id = parse_report(&json!({"kind":"full","items":[]}), revision(), None).unwrap();
    assert!(parse_report(&json!({"kind":"unchanged","resultId":"id"}), revision(), Some(&no_id)).is_err());
}

#[test]
fn malformed_reports_do_not_become_clean_diagnostic_results() {
    for raw in [
        Value::Null, json!({}), json!({"kind":"other","items":[]}),
        json!({"kind":"full"}), json!({"kind":"full","items":null}),
        ...
        json!({"kind":"unchanged"}), json!({"kind":"unchanged","resultId":"id","items":[]}),
    ] {
        assert!(parse_report(&raw, revision(), None).is_err(), "accepted {raw}");
    }
}

#[test]
fn item_byte_and_result_id_limits_apply_before_retaining_reports() {
    assert!(parse_report(&json!({"kind":"full","items":vec![diagnostic("x"); MAX_REPORT_ITEMS + 1]}), revision(), None).is_err());
    assert!(parse_report(&json!({"kind":"full","items":[diagnostic(&"x".repeat(MAX_REPORT_BYTES))]}), revision(), None).is_err());
    assert!(parse_report(&json!({"kind":"full","items":[],"resultId":"x".repeat(MAX_RESULT_ID_BYTES + 1)}), revision(), None).is_err());
    let value = json!(["☃"]);
...
fn report_cache_bounds_count_and_bytes_and_does_not_reuse_changed_versions() {
    ...
    let report = parse_report(&json!({"kind":"full","items":[],"resultId":"id"}), revision(), None).unwrap();
    for i in 0..MAX_REPORTS { assert!(cache.insert(i.to_string(), report.clone()).is_empty()); }
    ...
    assert!(cache.previous("extra", Revision { version:2, ..revision() }).is_none());
    let large = Report { bytes: MAX_CACHE_BYTES / 2 + 1, ..report };
...
fn pull_only_server_populates_existing_diagnostics_surface_and_reuses_result_ids() {
    ...
    assert!(peer.runtime.block_on(peer.client.wait_for_diagnostics(&uri, Duration::from_secs(5))));
    assert_eq!(peer.client.diagnostics_snapshot()[&uri][0]["message"], "broken");
    ...
    let pulls: Vec<_> = frames.iter().filter(|frame| frame["method"] == "textDocument/diagnostic").collect();
    ...
    let init = frames.iter().find(|frame| frame["method"] == "initialize").unwrap();
    assert_eq!(init["params"]["capabilities"]["textDocument"]["diagnostic"]["dynamicRegistration"], false);
    assert_eq!(init["params"]["capabilities"]["textDocument"]["diagnostic"]["relatedDocumentSupport"], false);
}
...
fn resynchronized_source_does_not_send_or_accept_an_old_result_baseline() {
    ...
    refresh(&peer, &uri).unwrap();
    assert_eq!(peer.client.diagnostics_snapshot()[&uri][0]["message"], "new");
    let frames = peer.frames();
    for frame in frames.iter().filter(|frame| frame["method"] == "textDocument/diagnostic").skip(1) {
...
fn a_newer_pull_supersedes_an_inflight_report_even_before_lane_admission() {
    ...
    peer.configure(json!({"textDocument/diagnostic":[{"hold":true}]}));
    peer.runtime.block_on(async {
        let mut first = Box::pin(peer.client.refresh_document_diagnostics(&uri, Duration::from_secs(5)));
        assert!(futures::poll!(first.as_mut()).is_pending());
        let mut second = Box::pin(peer.client.refresh_document_diagnostics(&uri, Duration::from_secs(5)));
...
fn failed_pull_preserves_previous_diagnostics_instead_of_clearing_them() {
    ...
    assert!(refresh(&peer, &uri).is_err());
    assert!(!peer.runtime.block_on(peer.client.wait_for_diagnostics(&uri, Duration::from_secs(5))));
    assert_eq!(peer.client.diagnostics_snapshot()[&uri][0]["message"], "retain me");
}
...
fn unsupported_unsynchronized_and_zero_budget_pulls_never_dispatch() {
    ...
    let (_, uri) = source(&peer, temp.path(), "source");
    assert!(refresh(&peer, &uri).is_err());
    assert!(!peer.frames().iter().any(|frame| frame["method"] == "textDocument/diagnostic"));
    let Some(pull_peer) = self::peer(temp.path()) else { return };
    assert!(refresh(&pull_peer, &uri).is_err());
    pull_peer.client.ensure_synced(&temp.path().join("source.rs"), "rust").unwrap();
    assert!(pull_peer.runtime.block_on(pull_peer.client.refresh_document_diagnostics(&uri, Duration::ZERO)).is_err());
    assert!(!pull_peer.frames().iter().any(|frame| frame["method"] == "textDocument/diagnostic"));
}
...
fn pull_timeouts_cancel_the_request_without_automatic_replay() {
    ...
    let (_, uri) = source(&peer, temp.path(), "source");
    peer.configure(json!({"textDocument/diagnostic":[{"hold":true}]}));
    let error = peer.runtime.block_on(peer.client.refresh_document_diagnostics(&uri, Duration::from_millis(30))).unwrap_err();
    assert!(error.to_string().contains("LSP_TIMEOUT"), "{error}");
    assert!(!peer.client.diagnostics_snapshot().contains_key(&uri));
    let frames = peer.frames();
    assert_eq!(frames.iter().filter(|frame| frame["method"] == "textDocument/diagnostic").count(), 1);
    assert_eq!(frames.iter().filter(|frame| frame["method"] == "$/cancelRequest").count(), 1);
}
