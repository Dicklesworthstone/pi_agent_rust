...
fn synchronization_capability_forms_and_defaults() {
    assert_eq!(SyncPolicy::parse(&json!({})).unwrap(), SyncPolicy::default());
    for kind in 0..=2 {
        assert_eq!(SyncPolicy::parse(&json!({"textDocumentSync":kind})).unwrap(),
            SyncPolicy { change: kind, open_close: kind != 0, save: None });
    }
    assert_eq!(SyncPolicy::parse(&json!({"textDocumentSync":{}})).unwrap(), SyncPolicy::default());
    for (save, expected) in [(json!(false), None), (json!(true), Some(false)),
        (json!({}), Some(false)), (json!({"includeText":true}), Some(true))]
    {
        assert_eq!(SyncPolicy::parse(&json!({"textDocumentSync":{
            "openClose":true,"change":2,"save":save
        }})).unwrap(), SyncPolicy { change: 2, open_close: true, save: expected });
    }
}

#[test]
fn invalid_sync_policies_are_not_guessed() {
    for sync in [json!(-1), json!(3), json!(false), json!(null), json!("2"),
        json!({"change":3}), json!({"change":true}), json!({"openClose":"yes"}),
        json!({"save":0}), json!({"save":{"includeText":"yes"}})]
    {
        assert!(SyncPolicy::parse(&json!({"textDocumentSync":sync})).is_err(), "{sync}");
    }
...
fn reference_offset(text: &str, wanted: &Value) -> usize {
    let (wanted_line, wanted_column) = (wanted["line"].as_u64().unwrap(), wanted["character"].as_u64().unwrap());
    let (mut line, mut column, mut offset) = (0, 0, 0);
    while offset < text.len() {
        if (line, column) == (wanted_line, wanted_column) { return offset; }
        let rest = &text[offset..];
        if rest.starts_with("\r\n") {
            offset += 2; line += 1; column = 0;
        } else {
            let ch = rest.chars().next().unwrap();
            offset += ch.len_utf8();
            if matches!(ch, '\n' | '\r') { line += 1; column = 0; }
            else { column += ch.len_utf16() as u64; }
        }
    }
    assert_eq!((line, column), (wanted_line, wanted_column), "invalid position in {text:?}");
    offset
}
...
fn replay_change(before: &str, change: &Value) -> String {
    ...
    format!("{}{}{}", &before[..start], change["text"].as_str().unwrap(), &before[end..])
}

#[test]
fn incremental_edits_roundtrip_unicode_and_every_line_ending() {
    let samples = ["", "abc", "abXc", "😀x", "😃x", "a\rb", "a\r\nb", "a\nb", "\r", "\r\n", "\n", "☃\r\n😀\nlast", "e\u{301}"];
    for before in samples {
        for after in samples {
            let change = incremental_change(before, after);
            assert_eq!(replay_change(before, &change), after, "{before:?} -> {after:?}: {change}");
        }
    }
}
...
fn incremental_edits_send_only_the_changed_span_and_utf16_positions() {
    assert_eq!(incremental_change("prefix😀Xsuffix", "prefix😀Ysuffix"), json!({
...
fn source_admission_rejects_large_files_and_directories() {
    ...
    assert!(read_document(&path).unwrap_err().to_string().contains("LSP_DOCUMENT_LIMIT"));
    assert!(read_document(temp.path()).unwrap_err().to_string().contains("LSP_FILE_UNREADABLE"));
}
...
"#;
    ...
    fn connect(root: &Path, caps: Value, runtime: &asupersync::runtime::Runtime) -> LspClient {
        runtime.block_on(LspClient::connect("python3",
            &["-u".to_string(), "-c".to_string(), SERVER.to_string(), caps.to_string()],
            &[], root, None, Duration::from_secs(5))).unwrap()
    }

    fn frames(client: &LspClient, runtime: &asupersync::runtime::Runtime) -> Vec<Value> {
        runtime.block_on(client.call("test/frames", json!({}), Duration::from_secs(5))).unwrap()
            .as_array().unwrap().iter().filter(|message| message["method"] != "initialized").cloned().collect()
    }
    ...
    fn full_sync_updates_without_reopening_and_sends_requested_save_text() {
        let temp = tempfile::tempdir().unwrap();
        let rt = runtime();
        let client = connect(temp.path(), json!({"textDocumentSync":{"openClose":true,"change":1,"save":{"includeText":true}}}), &rt);
        ...
        assert_eq!(messages.iter().map(|m| m["method"].as_str().unwrap()).collect::<Vec<_>>(),
            ["textDocument/didOpen", "textDocument/didChange", "textDocument/didSave"]);
        assert_eq!(messages[1]["params"]["contentChanges"], json!([{"text":"after😀"}]));
        assert_eq!(messages[2]["params"], json!({"textDocument":{"uri":uri},"text":"after😀"}));
    ...
    fn incremental_sync_preserves_a_live_document_and_reopens_only_for_language_changes() {
        ...
        assert_eq!(replay_change(before, &messages[1]["params"]["contentChanges"][0]), after);
    ...
    fn save_without_include_text_and_open_only_fallback_follow_server_options() {
        let temp = tempfile::tempdir().unwrap();
        let rt = runtime();
        let client = connect(temp.path(), json!({"textDocumentSync":{"openClose":true,"save":true}}), &rt);
        let path = source(temp.path(), "first");
        client.ensure_synced(&path, "plaintext").unwrap();
        std::fs::write(&path, "second").unwrap();
        client.ensure_synced(&path, "plaintext").unwrap();
        let messages = frames(&client, &rt);
        assert_eq!(messages.iter().map(|m| m["method"].as_str().unwrap()).collect::<Vec<_>>(),
            ["textDocument/didOpen", "textDocument/didClose", "textDocument/didOpen", "textDocument/didSave"]);
    ...
    fn versioned_diagnostics_cannot_survive_document_changes_or_close() {
        ...
        let publish = |version: Value| {
            rt.block_on(client.call("test/publish", json!({"uri":uri,"version":version,"diagnostics":[{"message":"fixture"}]}), Duration::from_secs(5))).unwrap();
        ...
        for invalid in [json!(old_version), json!(version + 1), json!("2"), json!(-1)] {
    ...
    fn source_and_version_limits_fail_before_sending_a_new_baseline() {
        ...
        client.next_document_version.store(u64::MAX, Ordering::SeqCst);
        std::fs::write(&path, "second").unwrap();
        assert!(client.ensure_synced(&path, "plaintext").unwrap_err().to_string().contains("LSP_VERSION_EXHAUSTED"));
    ...
    fn source_cache_eviction_closes_the_oldest_document_and_reopen_uses_a_new_version() {
        ...
        let messages = frames(&client, &rt);
        assert!(messages.iter().any(|m| m["method"] == "textDocument/didClose" && m["params"]["textDocument"]["uri"] == uri));
