...
fn inferred_type_and_argument_hints_reach_the_agent_as_read_only_metadata() {
    ...
    let Some(tool) = fixture(temp.path(),"hints") else { return };
    let payload = runtime().block_on(tool.execute("hints",request(false),None)).unwrap().details.unwrap();
    assert_eq!(payload["count"],3); assert_eq!(payload["readOnly"],true);
    assert_eq!(payload["range"],json!({"start":{"line":0,"character":0},"end":{"line":2,"character":0}}));
    ...
    assert!(!log.iter().any(|event|event["method"]=="inlayHint/resolve" || event["method"]=="workspace/executeCommand"));
    let init = log.iter().find(|event|event["method"]=="initialize").unwrap();
    assert_eq!(init.pointer("/params/capabilities/textDocument/inlayHint/resolveSupport/properties"),
...
fn exact_selected_ranges_preserve_same_position_hint_order() {
    ...
    let payload = runtime().block_on(tool.execute("range",input.clone(),None)).unwrap().details.unwrap();
    assert_eq!(payload["count"],2);
    assert_eq!(payload["hints"][0]["label"],": bool"); assert_eq!(payload["hints"][1]["label"],"inferred");
    assert_eq!(payload["hints"][0]["position"],payload["hints"][1]["position"]);
    let log = events(temp.path());
    let call = log.iter().find(|event|event["method"]=="textDocument/inlayHint").unwrap();
...
fn lazy_details_resolve_using_the_exact_original_opaque_hint() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(),"hints") else { return };
    let mut input = request(true); input["limit"] = json!(1);
    let payload = runtime().block_on(tool.execute("resolved",input,None)).unwrap().details.unwrap();
    assert_eq!(payload["hints"][0]["resolved"],true);
    assert_eq!(payload["hints"][0]["tooltip"]["value"],"resolved detail");
    assert_eq!(payload["hints"][0]["labelParts"][0]["tooltip"],"resolved part");
    assert_eq!(payload["hints"][0]["labelParts"][0]["location"]["uri"],"https://invalid.example/type");
    let log = events(temp.path());
    let calls: Vec<_> = log.iter().filter(|event|event["method"]=="inlayHint/resolve").collect();
    assert_eq!(calls.len(),1);
    let params = &calls[0]["params"];
    assert_eq!(params["data"],json!({"token":["opaque",7]}));
    assert!(params.get("index").is_none()); assert!(params.get("resolved").is_none());
...
fn only_retained_hints_are_resolved_and_truncation_is_explicit() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(),"hints-many") else { return };
    let mut input = request(true); input["limit"] = json!(2);
    let payload = runtime().block_on(tool.execute("many",input,None)).unwrap().details.unwrap();
    assert_eq!(payload["count"],2); assert_eq!(payload["total"],160); assert_eq!(payload["truncated"],true);
    let log = events(temp.path());
    let calls: Vec<_> = log.iter().filter(|event|event["method"]=="inlayHint/resolve").collect();
...
fn empty_results_and_inline_only_providers_remain_supported() {
    for mode in ["hints-null","hints-empty","hints-no-resolve","hints-boolean"] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(),mode) else { return };
        let payload = runtime().block_on(tool.execute("empty",request(false),None)).unwrap().details.unwrap();
        let empty = matches!(mode,"hints-null"|"hints-empty");
        assert_eq!(payload["count"],if empty {json!(0)} else {json!(3)});
        if !empty { assert_eq!(payload["resolveSupported"],false); }
    }
}
...
fn requesting_unavailable_resolution_does_not_send_a_hint_or_resolve_request() {
    for mode in ["hints-no-resolve","hints-boolean","hints-unsupported"] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(),mode) else { return };
        let error = runtime().block_on(tool.execute("unsupported",request(true),None)).unwrap_err();
        assert!(error.to_string().contains("LSP_UNSUPPORTED"),"{error}");
        assert!(!events(temp.path()).iter().any(|event|event["method"]=="textDocument/inlayHint" || event["method"]=="inlayHint/resolve"));
    }
}

#[test]
fn malformed_omitted_items_and_provider_failures_never_become_partial_success() {
    for (mode,code) in [("hints-error","hint engine failed"),("hints-malformed","LSP_SEMANTIC_MALFORMED"),
        ("hints-oversized","LSP_SEMANTIC_LIMIT"),("hints-outside","LSP_SEMANTIC_MALFORMED")] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(),mode) else { return };
        let mut input = request(true); input["limit"] = json!(1);
        input["range"] = json!({"start":{"line":1,"character":0},"end":{"line":1,"character":17}});
        let error = runtime().block_on(tool.execute("failure",input,None)).unwrap_err();
        assert!(error.to_string().contains(code),"{mode}: {error}");
        assert!(!events(temp.path()).iter().any(|event|event["method"]=="inlayHint/resolve"));
    }
}

#[test]
fn resolver_cannot_substitute_hints_or_introduce_unnegotiated_changes() {
    for (mode,code) in [("hints-resolve-change","LSP_SEMANTIC_MALFORMED"),("hints-resolve-data","LSP_SEMANTIC_MALFORMED"),
        ("hints-resolve-command","LSP_SEMANTIC_MALFORMED"),("hints-resolve-oversized","LSP_SEMANTIC_LIMIT"),
        ("hints-resolve-error","hint resolution failed")] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(),mode) else { return };
        let error = runtime().block_on(tool.execute("resolve-failure",request(true),None)).unwrap_err();
        assert!(error.to_string().contains(code),"{mode}: {error}");
        assert_eq!(std::fs::read_to_string(temp.path().join("source.pisig")).unwrap(),TEXT);
    }
}
...
fn source_drift_during_listing_or_lazy_resolution_retires_the_result() {
    for mode in ["hints-drift","hints-resolve-drift"] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(),mode) else { return };
        let error = runtime().block_on(tool.execute("drift",request(true),None)).unwrap_err();
        assert!(error.to_string().contains("LSP_SEMANTIC_STALE"),"{error}");
        assert_eq!(std::fs::read_to_string(temp.path().join("source.pisig")).unwrap(),"external hint edit\n");
    }
}
...
fn neither_hint_listing_nor_resolution_authorizes_workspace_edits() {
    for mode in ["hints-unsolicited","hints-resolve-unsolicited"] {
        ...
        let Some(tool) = fixture(temp.path(),mode) else { return };
        let mut input = request(true); input["limit"] = json!(1);
        runtime().block_on(tool.execute("unsolicited",input,None)).unwrap();
        let log = events(temp.path());
...
fn conflicting_selectors_and_invalid_ranges_fail_before_startup() {
    ...
    let Some(tool) = fixture(temp.path(),"hints") else { return };
    let rt = runtime();
    for (key,value) in [("position",json!({"line":1,"character":0})),("apply",json!(false)),("symbol",json!("call")),
        ("range",json!({"start":{"line":0,"character":4},"end":{"line":1,"character":0}})),("limit",json!(0))] {
        let mut input = request(false); input[key] = value;
    ...
    for action in ["signature_help","completion","status"] {
        ...
        let error = rt.block_on(tool.execute("invalid-action",input,None)).unwrap_err();
        assert!(error.to_string().contains("resolve is supported only"),"{error}");
    }
...
fn one_deadline_covers_hint_listing_and_lazy_resolution() {
    for mode in ["hints-hang","hints-resolve-hang"] {
        let temp = tempfile::tempdir().unwrap();
        let Some(tool) = fixture(temp.path(),mode) else { return };
        let mut input = request(true); input["timeout"] = json!(1);
        let started = Instant::now();
        let error = runtime().block_on(tool.execute("deadline",input,None)).unwrap_err();
...
fn aggregate_resolve_output_is_bounded_before_later_requests_are_issued() {
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(),"hints-resolve-output") else { return };
    let error = runtime().block_on(tool.execute("bounded",request(true),None)).unwrap_err();
    assert!(error.to_string().contains("LSP_SEMANTIC_LIMIT"),"{error}");
    let count = events(temp.path()).iter().filter(|event|event["method"]=="inlayHint/resolve").count();
    assert!((1..8).contains(&count),"must not resolve every item once output is exhausted: {count}");
}
...
fn dropping_a_lazy_hint_request_cancels_the_resolve_and_releases_the_lane() {
    ...
    use std::task::Poll;
    let temp = tempfile::tempdir().unwrap();
    let Some(tool) = fixture(temp.path(),"hints-resolve-hang") else { return };
    ...
    runtime().block_on(async {
        ...
        assert!(!tool.execute("after-drop",json!({"action":"status"}),None).await.unwrap().is_error);
    });
}
