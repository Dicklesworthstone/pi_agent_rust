...
#[derive(Debug)]
pub(super) enum Step {
    Text { path: PathBuf, edits: Range<usize> },
    File { index: usize, ignore_exists: bool, ignore_missing: bool },
}
...
impl Budget {
    fn step(&mut self) -> Result<()> {
        ...
        if self.steps > MAX_STEPS {
            return Err(plan_error("LSP_EDIT_LIMIT", "workspace edit exceeds 4096 operations"));
        }
        Ok(())
    }

    fn bytes(&mut self, bytes: usize) -> Result<()> {
        self.bytes = self.bytes.checked_add(bytes).filter(|size| *size <= MAX_PAYLOAD_BYTES)
    ...
    fn path(&mut self, raw: &Value, field: &str) -> Result<PathBuf> {
        let uri = raw.get(field).and_then(Value::as_str)
    ...
    fn text(&mut self, raw: &Value) -> Result<Vec<crate::lsp::text::TextEdit>> {
        let edits = raw.as_array().ok_or_else(|| malformed("edits is not an array"))?;
        self.edits = self.edits.checked_add(edits.len()).filter(|count| *count <= MAX_TEXT_EDITS)
            .ok_or_else(|| plan_error("LSP_EDIT_LIMIT", "workspace edit exceeds 32768 text edits"))?;
        for edit in edits {
...
fn option(entry: &Value, name: &str) -> Result<bool> {
    let Some(options) = entry.get("options") else { return Ok(false) };
    let options = options.as_object().ok_or_else(|| malformed("resource options must be an object"))?;
...
pub(super) fn parse(raw: &Value) -> Result<WorkspaceEditPlan> {
    ...
    if raw.is_null() { return Ok(plan); }
    let object = raw.as_object().ok_or_else(|| malformed("WorkspaceEdit is not an object"))?;
    ...
    if let Some(raw) = object.get("documentChanges") {
        let entries = raw.as_array().ok_or_else(|| malformed("documentChanges must be an array"))?;
        ...
        let changes = raw.as_object().ok_or_else(|| malformed("changes must be an object"))?;
...
fn parse_step(entry: &Value, plan: &mut WorkspaceEditPlan, budget: &mut Budget) -> Result<Step> {
    if !entry.is_object() { return Err(malformed("documentChanges entries must be objects")); }
    let Some(kind) = entry.get("kind") else {
        let document = entry.get("textDocument").ok_or_else(|| malformed("TextDocumentEdit missing textDocument"))?;
        ...
        if let Some(version) = document.get("version")
            && !version.is_null()
            && version.as_i64().and_then(|value| i32::try_from(value).ok()).is_none()
        {
            return Err(malformed("textDocument.version must be an integer or null"));
        }
        let path = budget.path(document, "uri")?;
        let raw_edits = entry.get("edits").ok_or_else(|| malformed("TextDocumentEdit missing edits"))?;
        let parsed = budget.text(raw_edits)?;
        let target = plan.text_edits.entry(path.clone()).or_default();
        let start = target.len();
        target.extend(parsed);
        return Ok(Step::Text { path, edits: start..target.len() });
    };
    let kind = kind.as_str().ok_or_else(|| malformed("documentChanges kind must be a string"))?;
    let (operation, ignore_exists, ignore_missing) = match kind {
        "create" => (FileOp::Create {
            path: budget.path(entry, "uri")?, overwrite: option(entry, "overwrite")?,
        }, option(entry, "ignoreIfExists")?, false),
        "rename" => (FileOp::Rename {
            old_path: budget.path(entry, "oldUri")?, new_path: budget.path(entry, "newUri")?,
        ...
        "delete" => {
            ...
            (FileOp::Delete { path: budget.path(entry, "uri")? }, false, option(entry, "ignoreIfNotExists")?)
        }
    ...
    Ok(Step::File { index, ignore_exists, ignore_missing })
}
...
fn validate_sequence(plan: &WorkspaceEditPlan, sequence: &[Step]) -> Result<()> {
    ...
    for step in sequence {
        match step {
            Step::Text { path, edits } => {
                let target = plan.text_edits.get(path).ok_or_else(|| malformed("ordered text target removed from plan"))?;
    ...
    if files != plan.file_ops.len() || offsets.len() != plan.text_edits.len()
        || plan.text_edits.iter().any(|(path, edits)| offsets.get(path.as_path()).copied() != Some(edits.len()))
    {
        return Err(malformed("plan contains changes not represented in document order"));
    }
    Ok(())
}
...
pub(super) fn stage(plan: &WorkspaceEditPlan) -> Result<Transaction> {
    ...
    if let Some(sequence) = &plan.sequence {
        ...
        for step in sequence {
            match step {
                Step::Text { path, edits } => transaction.edit(path, &plan.text_edits[path][edits.clone()])?,
                Step::File { index, ignore_exists, ignore_missing } => {
