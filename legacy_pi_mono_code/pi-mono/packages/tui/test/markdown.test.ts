...
3. Third item`,
      ...
      assert.strictEqual(
        ...
        3,
        `Expected 3 numbered items, got: ${numberedLines.join(", ")}`,
      );

      // Check the actual numbers
      assert.ok(
        numberedLines[0].startsWith("1."),
        `First item should be "1.", got: ${numberedLines[0]}`,
      );
      assert.ok(
        numberedLines[1].startsWith("2."),
        `Second item should be "2.", got: ${numberedLines[1]}`,
      );
      assert.ok(
        numberedLines[2].startsWith("3."),
        `Third item should be "3.", got: ${numberedLines[2]}`,
      );
    });
  });
...
bar`,
      ...
      assert.strictEqual(
        quotedLines.length,
        2,
        `Expected 2 quoted lines, got: ${JSON.stringify(plainLines)}`,
      );
      ...
      assert.ok(
        !fooLine?.includes("\x1b[35m"),
        `Foo line should NOT have magenta color: ${fooLine}`,
      );
      assert.ok(
        !barLine?.includes("\x1b[35m"),
        `bar line should NOT have magenta color: ${barLine}`,
      );
    });
...
>bar`,
    ...
    it("should wrap long blockquote lines and add border to each wrapped line", () => {
      ...
      assert.ok(
        ...
        `Expected multiple wrapped lines, got: ${JSON.stringify(contentLines)}`,
      );
