...
const isEnabled = (
  ...
  matchFn === "endsWith"
    ? r.path.endsWith(pathMatch) && r.enabled
    : r.path.includes(pathMatch) && r.enabled;
...
Content`,
  ...
  describe("resolveExtensionSources", () => {
    ...
    it("should handle directories with pi manifest", async () => {
      ...
      expect(
        result.extensions.some((r) => r.path === join(pkgDir, "src", "index.ts") && r.enabled),
      ).toBe(true);
      // Skills with SKILL.md are returned as file paths
      expect(
        result.skills.some(
          (r) => r.path === join(pkgDir, "skills", "my-skill", "SKILL.md") && r.enabled,
        ),
      ).toBe(true);
    });
  ...
  describe("pattern filtering in pi manifest", () => {
    it("should support glob patterns in manifest extensions", async () => {
      ...
      writeFileSync(
        join(pkgDir, "node_modules/dep/extensions", "remote.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "node_modules/dep/extensions", "skip.ts"),
        "export default function() {}",
      );
      writeFileSync(
        join(pkgDir, "package.json"),
  ...
  describe("force-include patterns", () => {
    ...
    it("should force-include multiple resources", async () => {
      ...
      writeFileSync(
        join(pkgDir, "skills/skill-a", "SKILL.md"),
        "---\nname: skill-a\ndescription: A\n---\nContent",
      );
      writeFileSync(
        join(pkgDir, "skills/skill-b", "SKILL.md"),
        "---\nname: skill-b\ndescription: B\n---\nContent",
      );
      writeFileSync(
        join(pkgDir, "skills/skill-c", "SKILL.md"),
        "---\nname: skill-c\ndescription: C\n---\nContent",
      );
...
export default function(api) { api.registerTool({ name: "test", description: "test", execute: async () => helper() }); }`,
      ...
      expect(result.extensions.some((r) => r.path.endsWith("subagent/index.ts") && r.enabled)).toBe(
        true,
      );
      expect(result.extensions.some((r) => r.path.endsWith("standalone.ts") && r.enabled)).toBe(
        true,
      );
    ...
    it("should respect package.json pi.extensions manifest in subdirectories", async () => {
      ...
      writeFileSync(
        join(pkgDir, "extensions", "custom", "main.ts"),
      ...
      expect(result.extensions.some((r) => r.path.endsWith("custom/main.ts") && r.enabled)).toBe(
        true,
      );
    ...
    it("should handle mixed top-level files and subdirectories", async () => {
      ...
      expect(result.extensions.some((r) => r.path.endsWith("complex/index.ts") && r.enabled)).toBe(
        true,
      );
