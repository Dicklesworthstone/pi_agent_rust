...
class UserMessageList implements Component {
  ...
  handleInput(keyData: string): void {
    ...
    if (kb.matches(keyData, "selectUp")) {
      this.selectedIndex =
        this.selectedIndex === 0 ? this.messages.length - 1 : this.selectedIndex - 1;
    }
    // Down arrow - go to next (newer) message, wrap to top when at bottom
    else if (kb.matches(keyData, "selectDown")) {
      this.selectedIndex =
        this.selectedIndex === this.messages.length - 1 ? 0 : this.selectedIndex + 1;
    }
...
export class UserMessageSelectorComponent extends Container {
  ...
  constructor(
    ...
    this.addChild(
      new Text(theme.fg("muted", "Select a message to create a new branch from that point"), 1, 0),
    );
    this.addChild(new Spacer(1));
