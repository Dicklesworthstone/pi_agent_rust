...
export class DefaultResourceLoader implements ResourceLoader {
  ...
  async reload(): Promise<void> {
    ...
    if (conflicts.length > 0) {
      ...
      extensionsResult.extensions = extensionsResult.extensions.filter(
        (ext) => !conflictingPaths.has(ext.path),
      );
    ...
    const promptPaths = this.noPromptTemplates
      ...
      : this.mergePaths(
          [...enabledPrompts, ...cliEnabledPrompts],
  ...
  private updateThemesFromPaths(
    ...
    const themePathsWithSource = this.themes.flatMap((theme) =>
      theme.sourcePath ? [theme.sourcePath] : [],
    );
  ...
  private applyExtensionMetadata(
    ...
    for (const resourcePath of resourcePaths) {
      ...
      const match = normalized.find(
        (entry) =>
          normalizedResourcePath === entry.path ||
