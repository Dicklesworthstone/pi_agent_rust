...
export const startModel = async (
  ...
  if (options.vllmArgs?.length) {
    ...
    if (options.gpus) {
      ...
      if (options.gpus > pod.gpus.length) {
        console.error(
          chalk.red(`Error: Requested ${options.gpus} GPUs but pod only has ${pod.gpus.length}`),
        );
        process.exit(1);
      }
      ...
      if (modelConfig) {
        ...
        console.error(
          chalk.red(
            `Model '${getModelName(modelId)}' does not have a configuration for ${options.gpus} GPU(s)`,
          ),
        );
      ...
      if (!modelConfig) {
        console.error(
          chalk.red(`Model '${getModelName(modelId)}' not compatible with this pod's GPUs`),
        );
        process.exit(1);
      }
    }
  ...
  if (!options.vllmArgs?.length) {
    ...
    if (options.context) {
      ...
      const maxTokens =
        contextSizes[options.context.toLowerCase()] || parseInt(options.context, 10);
...
export const showKnownModels = async () => {
  ...
  if (activePod) {
    ...
    console.log(
      chalk.bold(`Known Models for ${activePod.name} (${podGpuCount}x ${podGpuType || "GPU"}):\n`),
    );
  } else {
    console.log(chalk.bold("Known Models:\n"));
    console.log(
      chalk.yellow("No active pod. Use 'pi pods active <name>' to filter compatible models.\n"),
    );
  }
  ...
  const compatible: Record<
    string,
    Array<{ id: string; name: string; config: string; notes?: string }>
  > = {};
  const incompatible: Record<
    string,
    Array<{ id: string; name: string; minGpu: string; notes?: string }>
  > = {};
  ...
  for (const [modelId, info] of Object.entries(models)) {
    ...
    if (modelInfo.configs && modelInfo.configs.length > 0) {
      ...
      const sortedConfigs = [...modelInfo.configs].sort(
        (a: any, b: any) => (a.gpuCount || 1) - (b.gpuCount || 1),
      );
        ...
        for (const config of sortedConfigs) {
          ...
          if (configGpuCount <= podGpuCount) {
            ...
            if (
              ...
              configGpuTypes.some(
                (type: string) => podGpuType.includes(type) || type.includes(podGpuType),
              )
            ) {
              isCompatible = true;
              if (configGpuCount === 1) {
